...
if which.startswith("📗"):
  ...
  R->T; K->T; C->T; T->A; T->B; T->Cc; A->V; B->V; Cc->V; V->O;
    ...
    # ── ④ 검증 생성 (best-of-N rejection sampling) — 논문 Figure 1 뒤단 ──
    with st.container(border=True):
        st.subheader("④ 검증 생성 (best-of-N) — 생성→repair→기하·법규 검증→통과만 채택")
        st.caption("논문 프레임워크 뒤단: N개 후보 생성 후 geometric(strict clean=도면답게) + regulatory(채광 등 법규) "
                   "검증을 모두 통과한 도면만 채택. 통과율·기대 draws 보고(=논문 Table 5).")
        _bn = st.slider("후보 수 N", 4, 32, 8, help="많을수록 통과 도면 나올 확률↑·시간↑")
        _vgo = st.button("🛡 검증 생성 (best-of-N)", use_container_width=True)
        if _vgo:
            with st.spinner(f"{_bn}개 생성 → repair → 기하·법규 검증 중..."):
                try:
                    import torch
                    from pathlib import Path
                    from plan2graph import wallcycle_codec as wc
                    from plan2graph.generators.wall_cycle import WallCycleLM, make_constraint_mask
                    from plan2graph.gen_verify import best_of_n
                    dev = "cuda" if torch.cuda.is_available() else "cpu"
                    ckpt_path = Path(config.PROJECT_ROOT) / _mrow["ckpt"]
                    vocab_path = Path(config.DATA_DIR) / "staging" / _mrow.get("vocab", "tokens_korean_gated") / "vocab.json"
                    vocab = _json.load(open(vocab_path, encoding="utf-8"))
                    ...
                    def _gen1():
                        ...
                        return wc.canon_to_graph(wc.decode(row, vocab))

                    res = best_of_n(_gen1, n=_bn, repair=_use_repair)
                    s = res["stat"]
                    st.markdown("**검증 결과 (rejection sampling)**")
                    st.table([
                        {"Gate": "① geometric (strict clean·도면답게)", "통과": f"{s['geom_pass']}/{s['total']}"},
                        {"Gate": "② regulatory (법규·채광 등)", "통과": f"{s['legal_pass']}/{s['total']}"},
                        {"Gate": "③ both (clean + code)", "통과": f"{s['both_pass']}/{s['total']}  ({s['pass_rate']}%)"},
                    ])
                    _dr = s["expected_draws"]
                    st.caption(f"both 통과율 {s['pass_rate']}% → 1장 채택까지 기대 draws ≈ {_dr if _dr else '∞(이번 배치 0)'}. "
                               f"법규가 binding이면 N을 늘리거나 repair ON.")
                    if res["accepted"]:
                        st.success(f"✅ {len(res['accepted'])}장이 기하+법규 모두 통과 (채택)")
                    if res["best"]:
                        g_b, v_b = res["best"]
                        geom_b = _cr.autocorrect(_cr.from_geomgraph(g_b))
                        png_b = _cr.render_png(geom_b); dxf_b = _cr.render_dxf(geom_b)
                        st.markdown(f"##### 채택 도면  —  기하 {'✅' if v_b['geom_ok'] else '❌'} · 법규 {'✅' if v_b['legal_ok'] else '❌'}")
                        _vc1, _vc2 = st.columns(2)
                        _vc1.image(png_b, use_container_width=True)
                        _vc1.download_button("📥 PNG", png_b, file_name="verified.png", mime="image/png", use_container_width=True)
                        _vc2.download_button("📐 AutoCAD (DXF)", dxf_b, file_name="verified.dxf", mime="image/vnd.dxf", use_container_width=True)
                        if v_b["legal_violations"]:
                            _vc2.markdown("**법규 위반:**")
                            for _vi in v_b["legal_violations"][:6]:
                                _vc2.caption(f"- {_vi.get('rule')}: {_vi.get('msg', '')}")
                        else:
                            _vc2.success("법규 위반 0건 · 적용 규칙: " + ", ".join(v_b["legal_applied"]))
                    else:
                        st.warning("디코드된 후보가 없습니다. N을 늘려보세요.")
                except Exception as _ve:
                    st.error(f"검증 생성 실패: {type(_ve).__name__}: {_ve}")
                    import traceback
                    st.code(traceback.format_exc(), language="python")
