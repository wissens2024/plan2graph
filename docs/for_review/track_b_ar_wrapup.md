...
### 1-C. 한국 = pretrain(RPLAN ep50) → finetune (★n=200 확정)
| 모델 | valid(학습) | selfint=0 | overlap<.25 | 단일(벽2%) | clean(벽) |
|---|---|---|---|---|---|
| Korean-alone (pretrain無) | 0.47 | 10% | 83% | 71% | 9% |
| **finetune (pretrain)** | **0.69** | 12% | **91%** | **78%** | **12%** |
- **RPLAN pretrain 효과(n=200)**: valid 0.47→0.69(학습)·overlap 83→91%·단일벽 71→78%·clean 9→12% = **작지만 일관된 향상**(overlap/단일벽 SE 밖). lr 1e-4 vs 5e-5 미미.
- ★selfint(~10~12%)는 pretrain 불변 = **남은 병목**(§1-D).
...
- **생성물도 벽2%서 71~78% 단일**(n=200: 단독 71%·ft 78%) → 엔진이 응집된 벽-분리 아파트 생성.
- ★**진짜 병목 = selfint(방 폴리곤 자기교차) ~10~12%** (연결성·개구부 아님) → clean(벽인식) 9~12%(n=200).

---

## 2. 현재 버전에서 더 할 수 있는 것 (Python 후처리 — 재학습 없이)

### 2-A. 벽두께-인식 연결성 ✅검증완료 / selfint 수리 ❌실패 → rejection이 답
- ✅**연결성 = 문제 아님**(측정): GT 99%·생성 70~78% (벽2%, §1-D). 벽두께-인식 지표로 평가하면 한국 연결성 정상 — Python만으로 즉시 정정(재학습 0).
- ❌**selfint Python 수리는 효과 없음**(실측 정직한 음의 결과): buffer(0)·make_valid·직교 전부 clean +0~3pp(노이즈 이내). 이유 = 플랜당 14방 전부 깨끗해야 + `_self_intersects`가 make_valid보다 엄격. **수리는 레버 아님.**
- ★**진짜 레버 = rejection sampling**(§2-B). clean ~10~20% → 5~10회에 1개 "잘 나올 때까지". 수리보다 *재생성*이 AR엔 정답(생성 쌈·독립·고분산).
